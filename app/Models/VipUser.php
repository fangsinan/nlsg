...
where a.user_id = ' . $user_id . ' and a.status = 2
    ...
    public function vip360Info($vip_id, $user_id): array {
        $check = self::query()
            ->where('id', '=', $vip_id)
        ...
        if ($check->level === 1) {
            $res['is_360']      = 1;
            $res['start_time']  = date('Y-m-d', strtotime($check->start_time));
            $res['expire_time'] = date('Y-m-d', strtotime($check->expire_time));
        } else {
            if ($check->is_open_360 === 1) {
                $res['is_360']      = 1;
                $res['start_time']  = date('Y-m-d', strtotime($check->time_begin_360));
                $res['expire_time'] = date('Y-m-d', strtotime($check->time_end_360));
            }
        }
