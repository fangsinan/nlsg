...
//        if($type == 7 && $columnDatta['show_info_num'] > 0 ){
        ...
        if($type == 1 || $type == 6 || $type == 7){
            ...
            $works_info = DB::table('nlsg_column as w')
                ...
                ->select(['title','subtitle', 'subscribe_num', 'user_id', 'w.id', 'w.price', 'w.original_price' ,  'w.is_free', 'w.status','w.cover_pic as cover_img','w.comment_num','w.collection_num',
            ...
            ->where('w.id', '=', $works_id)
            ->select(['title','subtitle', 'subscribe_num', 'user_id', 'w.id', 'w.price', 'w.original_price', 'w.is_pay', 'w.type', 'w.is_free', 'w.status','w.cover_img','w.comment_num','w.collection_num','w.is_audio_book',
                DB::raw('if(s.id > 0,1,0) as is_sub')])
            ->first();
