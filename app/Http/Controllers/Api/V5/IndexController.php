...
//
//
//        while( count($list_arr) > 0 ){
//            // 开始搜索填充
//            $st = $list_arr[0];
//            array_shift($list_arr);
//
////            dump($st);
//            if($st == $end){  //最后一个元素时退出循环
//                break;
//            }
//
////            dd($list_arr);
//            //四个方向  上下左右
//            foreach ($top_star as $key=>$val){
//                $next = [ $st[0]+$val[0], $st[1]+$val[1],];
//
//                //在迷宫里  下一步撞墙   跳过本次步骤
//                if($next[0] < 0 || $next[0] >= count($arr)){
//                    continue;
//                }
//                if($next[1] < 0 || $next[1] >= count($arr[1])){
//                    continue;
//                }
//                //撞墙  == 1
...
//
//
//                // 在路线图 steps中 下个步骤不是0   的情况下依旧跳过
//                //  steps为路线图  初始默认值是0
//                if($next[0] < 0 || $next[0] >= count($res_arr)){
//                    continue;
//                }
//                if($next[1] < 0 || $next[1] >= count($res_arr[1])){
//                    continue;
//                }
//
//                if($res_arr[$next[0]][$next[1]] != 0){
//                    continue;
//                }
//
//                //下个步骤 如果回到起点 依然跳过
//                if($next == $star){ //回到起点时 结束
//                    continue;
//                }
//                //将走过的路线图  赋值到res_arr中
////                $st_res  = $res_arr[$next[0]][$next[1]];
//                $st_res  = $res_arr[$st[0]][$st[1]];
//                $res_arr[$next[0]][$next[1]] = $st_res+1;
//
//
//                //将通道加入下次路径
//                $list_arr[] = $next;
